...
namespace TXM.Core
{
    ...
    public class WarmachineRules : AbstractRules
    {
        ...
        protected override bool CalculateResult(Result result, Func<int, int, int> f)
        {
            ...
            var tP = newResult.WinnerID == -99 ? 0 : newResult.Destroyed - newResult.Lost;
        ...
        {
            var t = unsorted.OrderByDescending(x => x.IsInCut).ThenByDescending(x => x.Wins).ThenByDescending(x => x.StrengthOfSchedule).ThenByDescending(x => x.TournamentPoints).ThenBy(x => x.Order).ToList();
